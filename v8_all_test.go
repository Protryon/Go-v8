...
func Test_Accessor(t *testing.T) {
	engine.NewContext(nil).Scope(func(cs ContextScope) {
		...
		for i := 0; i < 2; i++ {
			...
			object.SetProperty("abc", cs.NewInteger(5678))
...
func Test_NamedPropertyHandler(t *testing.T) {
	...
	engine.NewContext(global_template).Scope(func(cs ContextScope) {
		...
		object.SetProperty("abc", cs.NewInteger(123))
...
func Test_Context(t *testing.T) {
	...
	//TODO: fix contexes
	/*
	script1 := engine.Compile([]byte("typeof(Test_Context) == 'undefined';"), nil)
	script2 := engine.Compile([]byte("Test_Context = 1;"), nil)
	script3 := engine.Compile([]byte("Test_Context = Test_Context + 7;"), nil)

	test_func := func(cs ContextScope) {
		if script1.Run().IsFalse() {
			t.Fatal(`script1.Run(c).IsFalse()`)
		}else{
			fmt.Println("testsfsdf")
		}
