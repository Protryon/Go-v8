...
void V8_Initialize(const char* executableLocation) {
	// Initialize V8.
	V8::InitializeICUDefaultLocation(executableLocation);
	V8::InitializeExternalStartupData(executableLocation);
...
void* V8_NewEngine() {
	...
	Isolate* tiso = Isolate::New(create_params);
	ISOLATE_SCOPE(tiso);
...
void V8_Context_Scope(void* context, void* context_ptr, void* callback) {
	...
	void* prev_context = isolate->GetData(0);
	scope_data data;
	...
	isolate->SetData(0, prev_context);
}
...
V8_Context* V8_Current_Context(Isolate* isolate) {
	...
	return static_cast<V8_Context*>(static_cast<scope_data*>(isolate->GetData(0))->context);
}
...
void* V8_Current_ContextPtr(Isolate* isolate) {
	...
	if (data == NULL)
		v8_panic((char*)"Please call this API in a context scope");
	return static_cast<scope_data*>(isolate->GetData(0))->context_ptr;
}
...
script
*/
void* V8_Compile(void* engine, const char* code, int length, void* script_origin) {
	ENGINE_SCOPE(engine);
	HandleScope handle_scope(isolate);
	MaybeLocal<Script> realScript = Script::Compile(isolate->GetCurrentContext(), String::NewFromOneByte(isolate, (uint8_t*)code, String::kNormalString, length), static_cast<ScriptOrigin*>(script_origin));
	if (realScript.IsEmpty())
		return NULL;
	return (void*)(new V8_Script(the_engine, realScript.ToLocalChecked()));
}
...
void* V8_NewObject(void* context) {
	...
	return new_V8_Value(the_context, Object::New(isolate));
}
...
int V8_Object_SetProperty(void* value, const char* key, int key_length, void* prop_value) {
	VALUE_SCOPE(value);
	bool result = false;
	Local<Object>::Cast(local_value)->Set(
...
void* V8_Function_NewCall(void* value, int argc, void* argv) {
	VALUE_SCOPE(value);
...
void* V8_NewFunctionTemplate(void* engine, void* callback, void* data) {
	...
	Handle<FunctionTemplate> tpl = callback == NULL ? FunctionTemplate::New(isolate) : FunctionTemplate::New(isolate, 
...
void V8_AddMessageListener(void* callback, void* data, int simple) {
	...
	Handle<Array> args = Array::New(Isolate::GetCurrent(), 3);
	args->Set(0, External::New(Isolate::GetCurrent(), callback));
	args->Set(1, External::New(Isolate::GetCurrent(), data));
	args->Set(2, Boolean::New(Isolate::GetCurrent(), simple));
